...
export default class Interface {
  ...
  buildStyle() {
    return el('style',
      ...
      '#cconsent-bar button.consent-reject { margin-right:15px }',
  ...
  addEventListeners(elements) {
    ...
    for (let button of buttonConsentReject) {
      button.addEventListener('click', () => {
        ...
        // We set config to full reject
        for (let key in window.CookieConsent.config.categories) {
          window.CookieConsent.config.categories[key].wanted = 
            window.CookieConsent.config.categories[key].checked = (window.CookieConsent.config.categories[key].needed === true) ? true : false;
        }
  ...
  setCookie(cookie, callback) {
    ...
    document.cookie = `cconsent=${JSON.stringify(cookie)}; expires=${expires_in}; path=/;SameSite=Lax`;
